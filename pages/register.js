...
const Register = () => {
  const { user, createUser, googleProviderSignIn } = useContext(AuthContext);
  console.log(user);
  ...
  const handleGoogleRegister = () => {
    googleProviderSignIn()
      .then(result => {
        const user = result.user
        console.log(user);
        toast.success('user sign up suceefully')
        router.push('/');
      })
      .catch(err => {
        console.log(err);
      ...
      <div className='w-96 p-7'>
        ...
        <form onSubmit={handleSubmit(handleSignUp)}>
          <div className="form-control w-full max-w-xs">
            ...
            <input type="text" placeholder='Enter Name' {...register("name", { required: 'Name is required' })} className="input input-bordered border-4 w-full my-4 max-w-xs" />
            {errors.name && <p>{errors.name.message
            }</p>}
          </div>

          <div className="form-control w-full max-w-xs">
            <label className="label"> <span className="label-text text-black py-4">Email</span></label>
            <input type="email" placeholder='Enter Email' {...register("email", {
              required: 'email is required'
            })} className="input input-bordered border-4 w-full my-4 px-2 max-w-xs" />
        ...
        <div className="flex justify-end gap-2 p-6 pt-0 my-4">
          <button onClick={handleGoogleRegister} className="inline-flex h-10 flex-1 items-center justify-center gap-2 justify-self-center whitespace-nowrap rounded bg-emerald-50 px-5 text-xs font-medium tracking-wide text-emerald-500 transition duration-300 hover:bg-emerald-100 hover:text-emerald-600 focus:bg-emerald-200 focus:text-emerald-700 focus-visible:outline-none disabled:cursor-not-allowed disabled:border-emerald-300 disabled:bg-emerald-100 disabled:text-emerald-400 disabled:shadow-none">CONTINUE WITH GOOGLE</button>
        </div>
      </div>
    </div>
  );
